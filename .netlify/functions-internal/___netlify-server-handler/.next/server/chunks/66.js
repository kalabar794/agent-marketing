exports.id=66,exports.ids=[66],exports.modules={6068:()=>{},6450:(a,b,c)=>{"use strict";c.d(b,{n:()=>q});class d{constructor(a){this.maxRetries=3,this.timeout=3e4,this.agentName=a}async callLLM(a,b){let{model:c="claude-3-sonnet-20240229",maxTokens:d=4e3,temperature:e=.7}=b||{},f={model:c,max_tokens:d,temperature:e,messages:[{role:"user",content:a}]},g=null;for(let a=1;a<=this.maxRetries;a++)try{let a=new AbortController,b=setTimeout(()=>a.abort(),this.timeout),c=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":process.env.ANTHROPIC_API_KEY||"","anthropic-version":"2023-06-01"},body:JSON.stringify(f),signal:a.signal});if(clearTimeout(b),!c.ok){let a=await c.json().catch(()=>({}));throw Error(`API request failed: ${c.status} ${c.statusText} - ${JSON.stringify(a)}`)}let d=await c.json();if(d.content&&d.content[0]&&d.content[0].text)return d.content[0].text;throw Error("Invalid response format from Claude API")}catch(b){if(g=b instanceof Error?b:Error(String(b)),b instanceof Error&&"AbortError"===b.name&&(g=Error(`Request timeout after ${this.timeout}ms`)),console.warn(`${this.agentName} LLM call attempt ${a} failed:`,g.message),a<this.maxRetries){let b=1e3*Math.pow(2,a);await new Promise(a=>setTimeout(a,b))}}throw Error(`${this.agentName} failed after ${this.maxRetries} attempts. Last error: ${g?.message}`)}async callLLMWithFallback(a,b){try{return await this.callLLM(a)}catch(a){return console.error(`${this.agentName} LLM call failed, using fallback:`,a),JSON.stringify(b)}}extractJSONFromResponse(a){try{let b=a.match(/\{[\s\S]*\}/);if(b)return JSON.parse(b[0]);return JSON.parse(a)}catch(a){throw Error(`Failed to extract JSON from response: ${a}`)}}validateRequiredFields(a,b){for(let c of b)if(!a[c])throw Error(`Missing required field: ${c}`)}sanitizeOutput(a){if("string"==typeof a)return a.replace(/<script[^>]*>.*?<\/script>/gi,"").replace(/javascript:/gi,"").replace(/on\w+\s*=/gi,"");if(Array.isArray(a))return a.map(a=>this.sanitizeOutput(a));if("object"==typeof a&&null!==a){let b={};for(let[c,d]of Object.entries(a))b[c]=this.sanitizeOutput(d);return b}return a}async healthCheck(){try{let a=`Respond with exactly this JSON: {"status": "healthy", "agent": "${this.agentName}"}`,b=await this.callLLM(a,{maxTokens:100,temperature:0}),c=this.extractJSONFromResponse(b);return"healthy"===c.status&&c.agent===this.agentName}catch(a){return console.error(`Health check failed for ${this.agentName}:`,a),!1}}logExecution(a,b){console.log(`[${this.agentName}] ${a}`,b?JSON.stringify(b,null,2):"")}getExecutionTimestamp(){return new Date().toISOString()}}class e extends d{async execute(a,b){let c=this.buildPrompt(a);try{let a=await this.callLLM(c);return this.parseResponse(a)}catch(a){throw Error(`Market research failed: ${a}`)}}buildPrompt(a){return`You are a professional market researcher specializing in ${this.getIndustryFromTopic(a.topic)}. 
